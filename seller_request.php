...
if(isset($_POST['Submit']))
{
   ...
   if(strlen($_POST['Username']) < 1 || strlen($_POST['Password']) < 1 || 
    ...
    $qry = "SELECT * FROM prologin where Username ='$user' or Email='$email'";
    ...
    {
        $_SESSION['error'] = 'Already taken';
        header("Location:seller_request.php");
        return;
    }

    $qry = "SELECT * FROM pending_request where Email='$email' and Type='Seller'";
    $result = mysqli_query($link,$qry);
    if(mysqli_num_rows($result) == 1)
    {
      $_SESSION['error'] = 'We have already send your request .Sorry! for delay';
       ...
       $message = "Name: ".$name." is eager to get into our small family(seller side) please check if he/she is a valid user"."\n"." His email id is ".$email."\n"."Institute Roll No is ".$roll_no."\n"."Requested Username ".$user."\n"." Requested Password ".$password;
       ...
       {
         $qry = "INSERT INTO pending_request(Email,Type,College,Roll_No) VALUES('$email','Seller','$College',$roll_no)";
         $result = mysqli_query($link,$qry);
...
<body>
  ...
  <div class="fi" >
    <label>
      <span>Username (Preffered)</span>
      <input name="Username" type="text" autocomplete="off" required>
    </label>
  </div>
  <div class="fi" >
    <label>
      <span>Password (Preffered)</span>
